...
class task_model {
	...
	*/
	...
	public static function get($task_id) {
		$sql = "SELECT * FROM task LEFT JOIN user ON task.user_id = user.user_id LEFT JOIN task_template ON task.tt_id = task_template.tt_id LEFT JOIN year_week ON task.yw_id = year_week.yw_id LEFT JOIN attempt ON task.attempt_id = attempt.attempt_id LEFT JOIN domain ON user.domain_id = domain.domain_id LEFT JOIN activity_template ON task_template.at_id = activity_template.at_id LEFT JOIN year_level ON task_template.yl_id = year_level.yl_id LEFT JOIN activity ON attempt.activity_id = activity.activity_id LEFT JOIN school ON domain.school_id = school.school_id LEFT JOIN district ON year_level.district_id = district.district_id LEFT JOIN country ON district.country_iso = country.country_iso WHERE task.task_id='%s'";
	...
	public static function list_by_attempt_id($attempt_id) {
		$sql = "SELECT * FROM task LEFT JOIN user ON task.user_id = user.user_id LEFT JOIN task_template ON task.tt_id = task_template.tt_id LEFT JOIN year_week ON task.yw_id = year_week.yw_id LEFT JOIN attempt ON task.attempt_id = attempt.attempt_id LEFT JOIN domain ON user.domain_id = domain.domain_id LEFT JOIN activity_template ON task_template.at_id = activity_template.at_id LEFT JOIN year_level ON task_template.yl_id = year_level.yl_id LEFT JOIN activity ON attempt.activity_id = activity.activity_id LEFT JOIN school ON domain.school_id = school.school_id LEFT JOIN district ON year_level.district_id = district.district_id LEFT JOIN country ON district.country_iso = country.country_iso WHERE task.attempt_id='%s';";
	...
	public function insert() {
		$sql = "INSERT INTO task(user_id, task_due, task_complete, task_grade, yw_id, attempt_id, tt_id) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s');";
		return database::insert($sql, array($this -> user_id, $this -> task_due, $this -> task_complete, $this -> task_grade, $this -> yw_id, $this -> attempt_id, $this -> tt_id));
	}

	public function update() {
		$sql = "UPDATE task SET user_id, task_due, task_complete, task_grade, yw_id, attempt_id, tt_id WHERE task_id ='%s';";
		return database::update($sql, array($this -> user_id, $this -> task_due, $this -> task_complete, $this -> task_grade, $this -> yw_id, $this -> attempt_id, $this -> tt_id, $this -> task_id));
	}
}
?>